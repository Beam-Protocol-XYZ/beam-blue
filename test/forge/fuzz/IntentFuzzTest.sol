...
contract IntentFuzzTest is BaseTest {
    ...
    function testFuzz_MatchIntents(
        ...
        uint256 collateralAmount = loanAmount.mulDivUp(1e36, collateralPrice).mulDivUp(1e18, requiredLTV);
        ...
        IIntentLending.MatchedLoan memory loan = intentLending.getLoan(
            keccak256(abi.encodePacked(lendId, borrowId, loanAmount, uint256(0)))
        );
    ...
    function testFuzz_Liquidation(
        ...
        bytes32 loanId = intentLending.matchIntents(lendId, borrowId, 0.07e18, loanAmount);
        ...
        if (health < 1e18) {
            ...
            assertFalse(intentLending.getLoan(loanId).active, "Loan should be inactive after liquidation");
        } else {
